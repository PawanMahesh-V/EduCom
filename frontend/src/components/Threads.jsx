...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, ...rest }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      function update(t) {
        const ux = 0.5 + 0.12 * Math.sin(t * 0.0006);
        const uy = 0.5 + 0.10 * Math.cos(t * 0.0004);
        program.uniforms.uMouse.value[0] = ux;
        program.uniforms.uMouse.value[1] = uy;
  ...
  return (
    <div 
      ref={containerRef} 
      className={`threads-container ${!webglSupported ? 'threads-fallback' : ''}`}
      {...rest} 
    />
  );
};  
